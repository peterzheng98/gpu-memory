...
from rednet_changed.vit import vit_b_16 as vit_b_16_rednet
from rednet_changed.resnet import resnet50 as resnet50_rednet
...
def prepare_model(modelname: str):
    if modelname == "vit":
        model = vit_b_16_rednet()
        std_model = torchvision.models.vit_b_16(pretrained=True)
    else:
        model = resnet50_rednet()
        std_model = torchvision.models.resnet50(pretrained=True)
    weight_dict = model.state_dict()
    for key, value in std_model.state_dict().items():
...
def eval(model: torch.nn.Module, std_model: torch.nn.Module, testloader: torch.utils.data.DataLoader, args):
    ...
    with torch.no_grad():
        ...
        print(f'[*] RedNet to normalized: {target_acc / baseline_acc}')
